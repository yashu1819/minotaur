...
/**
 * \file RoundingHeur.cpp
 * \brief Define the RoundingHeur class for diving heuristics for MINLPs.
 * \author Prashant Palkar, IIT Delhi
 *
 * Implements the class RoundingHeur.
 */
...
const std::string RoundingHeur::me_ = "ParMINLP Diving Heuristic: "; 

RoundingHeur::RoundingHeur(EnvPtr env, ProblemPtr p, EnginePtr e)
: e_(e), 
  env_(env), 
  //gradientObj_(NULL),
  intTol_(1e-5),
...
void RoundingHeur::backtrack_(UInt n_moded, ModVector& lastNodeMods, ProblemPtr p,
...
UInt RoundingHeur::FracBounds_(UInt numfrac, const double* x, Direction d,
...
void RoundingHeur::getScore_(const double* x, Scoretype s,
   ...
   case (LexBound)   :
     for (VariableConstIterator v_iter=p->varsBegin();
       ...
       if (variable->getType() == Binary || variable->getType() == Integer) {
         ...
         if (fractional > RoundingHeur::intTol_) {
           score.push_back(i);
         }
       }
     }
...
void RoundingHeur::implementDive_(int i, const double* x, SolutionPoolPtr s_pool,
                                    ...
                                    DivingheurStats* stats)
{
  ...
  if (f == &RoundingHeur::VectorLength_) {
...
UInt RoundingHeur::isFrac_(const double* x, UIntVector& violated, ProblemPtr p)
{
  VariablePtr variable;
  double value;
  UInt i = 0;
...
void RoundingHeur::solveNLP(ConstSolutionPtr sol, bool *solFound,
...
void RoundingHeur::fixInts(const double *x,
                             std::stack<Modification *> &nlpMods,
                             ProblemPtr minlp)
{
  double xval;
...
void RoundingHeur::unfixInts(std::stack<Modification *> &nlpMods,
                               ProblemPtr minlp)
{
  Modification *m = 0;
...
UInt RoundingHeur::LexBounds_(UInt numfrac, const double* x, Direction d,
                                Order o, ProblemPtr p, UIntVector& violated,
                                std::stack<VarBoundModPtr>& mods,
                                LinearHandler* lh, ModVector& lastNodeMods,
                                DoubleVector& score, DoubleVector& avgDual,
...
UInt RoundingHeur::ReducedCost_(UInt numfrac, const double* x, Direction d,
                                  Order o, ProblemPtr p, UIntVector& violated,
                                  std::stack<VarBoundModPtr>& mods,
                                  LinearHandler* lh, ModVector& lastNodeMods,
                                  DoubleVector& score, DoubleVector& avgDual,
...
void RoundingHeur::restoreBounds_(double* LB_copy, double* UB_copy, UInt vars, ProblemPtr p)
{
...
double RoundingHeur::rounding_(double value, Direction d)
{
  switch (d) {
...
void RoundingHeur::saveBounds_(double* LB_copy, double* UB_copy, UInt vars)
{
  VariablePtr variable;
  for (UInt i=0; i<vars; ++i, ++LB_copy, ++UB_copy) {
...
RoundingHeur::FuncPtr RoundingHeur::selectHeur_(int i, Direction &d, Order &o)
{
#if USE_OPENMP
#pragma omp critical (log)
  ...
  if (i < 8) {
    return &RoundingHeur::FracBounds_; 
  } else if (i < 16) {
    return &RoundingHeur::VectorLength_;
  } else if (i < 24) {
    return &RoundingHeur::ReducedCost_;
  } else {
    return &RoundingHeur::LexBounds_;
  } 
}


bool RoundingHeur::shouldDive_()
{
  UInt min_num_bint             = 5;
  ...
  switch (option) {
   ...
   default : assert(!"Unknown option for RoundingHeur heuristic");
             return false;
             break;
  }
...
void RoundingHeur::solve(NodePtr, RelaxationPtr, SolutionPoolPtr s_pool)
{
  bool goAhead = true;
...
void RoundingHeur::sort_(UInt left, UInt right, DoubleVector& score, UIntVector& violated)
{
  UInt i = left;
  UInt j = right;
...
void RoundingHeur::updateAvgDual_(ConstSolutionPtr sol,
...
UInt RoundingHeur::VectorLength_(UInt numfrac, const double* x, Direction d,
                                   Order o, ProblemPtr p, UIntVector& violated,
                                   std::stack<VarBoundModPtr>& mods,
                                   LinearHandler* lh, ModVector& lastNodeMods,
                                   DoubleVector& score, DoubleVector& avgDual,
...
bool RoundingHeur::vectorFlag_(UInt min_vlength, ProblemPtr p)
{
  UInt num_obj_int      = 0;
...
void RoundingHeur::writeParStats(std::ostream &out, DivingheurStats* stats,
...
void RoundingHeur::writeStats(std::ostream &out) const
{
...
std::string RoundingHeur::getDirectionString(UInt i)
{
  switch (i%4) {
   case 0:
     return "Floor";
...
std::string RoundingHeur::getOrderString(UInt i)
{
  if (i%8 < 4) {
...
std::string RoundingHeur::getScoreString(UInt i)
{
  switch (i/8) {
   case 0:
     return "Fractional";
